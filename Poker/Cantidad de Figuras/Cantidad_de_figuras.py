import pandas as pd
from urllib.request import urlopen
import io
import time
import trace
import sys

def descargar_datos():
    url = "https://archive.ics.uci.edu/ml/machine-learning-databases/poker/poker-hand-training-true.data"
    response = urlopen(url)
    data = response.read().decode('utf-8')
    column_names = ['S1','C1','S2','C2','S3','C3','S4','C4','S5','C5','CLASS']
    return pd.read_csv(io.StringIO(data), header=None, names=column_names)

def contar_cartas_especiales_rolling():
    start_time = time.time()
    df = descargar_datos()
    ventana_size = 10
    columnas_cartas = ['C1','C2','C3','C4','C5']
    resultados = []

    for i in range(0, len(df), ventana_size):
        ventana = df.iloc[i:i+ventana_size]
        if len(ventana) > 0:
            conteo_series = ventana[columnas_cartas].stack().value_counts()
            resultados.append({
                'ventana_id': i//ventana_size+1,
                'inicio_fila': i,
                'fin_fila': i+len(ventana)-1,
                'J': conteo_series.get(11, 0),
                'Q': conteo_series.get(12, 0),
                'K': conteo_series.get(13, 0),
                'A': conteo_series.get(1, 0),
                'total_cartas': len(ventana)*5
            })

    df_conteo = pd.DataFrame(resultados)
    df_conteo.to_csv('conteo_cartas.csv', index=False)
    end_time = time.time()
    print(df_conteo.head())
    print(f"\nTiempo de ejecución: {end_time - start_time:.2f} s")

if __name__ == "__main__":
    tracer = trace.Trace(trace=True, count=False)
    with open("trace_conteo_cartas.txt","w") as f:
        old_stdout = sys.stdout
        sys.stdout = f
        tracer.runfunc(contar_cartas_especiales_rolling)
        sys.stdout = old_stdout
    contar_cartas_especiales_rolling()

