...
def encontrar_jugada_ganadora():
    ...
    df_ganadoras.to_csv('jugadas_ganadoras.csv', index=False)

    end_time = time.time()

    # Mostrar solo el resultado final y el tiempo
    print(df_ganadoras.head())
    print(f"\nTiempo de ejecución: {end_time - start_time:.2f} s")

if __name__ == "__main__":
    # Crear el trace en un archivo, sin que aparezca en pantalla
    tracer = trace.Trace(trace=True, count=False)
    with open("trace_jugada_ganadora.txt", "w") as f:
        old_stdout = sys.stdout
        sys.stdout = f
        tracer.runfunc(encontrar_jugada_ganadora)
        sys.stdout = old_stdout

    # Ejecutar la función normalmente para mostrar el DataFrame y tiempo
    encontrar_jugada_ganadora()
